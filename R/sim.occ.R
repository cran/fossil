`sim.occ` <-
function(total.species = 100, endemics = 0.1, regions = 3, locs = 30, avg.abund = 100) {
    ...
    for (i in 1:(regions*locs)) {
        ...
        orig.groups[orig.groups[,i]==1,i]<-round(rlnorm(length(orig.groups[orig.groups[,i]==1,i]), 0, 1))
